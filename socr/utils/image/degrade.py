## From https://github.com/tmbdev/ocropy2/blob/master/ocropy2/degrade.py
...
def gauss_degrade(image, margin=1.0, change=None, noise=0.02, minmargin=0.5, inner=1.0):
    ...
    if ri <= 0:
        mask = binary_dilation(image, iterations=r) - image
    else:
        mask = binary_dilation(image, iterations=r) - binary_erosion(image, iterations=ri)
...
def gauss_distort(images, maxdelta=4.0, sigma=20.0):
    n, m = images[0].shape
    deltas = randn(2, n, m)
